...
def _run_project(run_command: str, project_dir: Path, timeout: int = 30) -> str:
    ...
    if index_html.exists() and run_command.lower().startswith(("open", "browser", "preview")):
        try:
            webbrowser.open(f"file:///{index_html.absolute()}")
            return f"Web project previewed: {index_html.name} opened in browser."
        except Exception as e:
...
def _build_project(
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        if not _has_error(last_output, run_command):
            ...
            # Special auto-preview for web projects
            if (project_dir / "index.html").exists() and not run_command.lower().startswith(("open", "browser", "preview")):
                log("Auto-previewing web project...")
                webbrowser.open(f"file:///{(project_dir / 'index.html').absolute()}")
