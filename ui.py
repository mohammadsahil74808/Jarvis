import os, json, time, math, random, threading, psutil
...
class JarvisUI:
        ...
        def _update_loop():
            while True:
                try:
                    ...
                    diff = (curr_bytes - self.last_net_bytes) / 1024.0 # in KB
    ...
    def _draw(self):
        ...
        # Left side: Restore Original Badge
        c.create_text(16, 31, text=MODEL_BADGE,
                      fill=C_DIM, font=("Courier", 9), anchor="w")
        
        # Right side: 12-hour Time Only
        t12 = time.strftime("%I:%M:%S %p")
        c.create_text(W - 16, 31, text=t12,
